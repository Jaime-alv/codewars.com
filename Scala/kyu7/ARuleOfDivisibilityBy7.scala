...
object ARuleOfDivisibilityBy7 extends App {
  /*
  ...
  */
  ...
  def seven(m: Long, rounds: Int = 0): Array[Int] = {
    if (m == 0) Array(0, rounds)
    else {
      val checking = m.toString.dropRight(1).toLong - (m.toString.takeRight(1).toLong * 2)
      if (checking.toString.length <= 2) Array(checking.toInt, rounds + 1)
      else seven(checking, rounds + 1)
    }
  }

  println(seven(188958445315636L).mkString(","))
}
