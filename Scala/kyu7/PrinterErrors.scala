...
object PrinterErrors extends App{
  ...
  def printerErrors(s: String): Any = {
    f"${s.length - s.map(x => ('a' to 'm').count(_ == x)).sum}/${s.length}"
  }

  def printerEasier(s: String): String =
    s"${s.count(_ > 'm')}/${s.length}"

  println(printerErrors("anaabbbbhaijjjmx"))
}
