...
object FindTheMissingLetter extends App{
  ...
  */
  ...
  def findTheMissingLetter(chars: Array[Char]): Char = {
    (chars(0) to chars(chars.length - 1)).filterNot(chars.contains(_))(0)
  }

  println(findTheMissingLetter(Array('a','b','c','d','f')))
}
