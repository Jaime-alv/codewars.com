...
object TribonacciSequence extends App{
  /*
  ...
  def tribonacci[T : Numeric](signature: List[T], n: Int): List[T] = {
    ...
    // else if (signature.length == n) signature
    else {
  ...
  def fibonacci(signature: List[Int], n: Int): List[Int] = {
    if (signature.length == n) signature
    else {
      fibonacci(signature.:+ {
        signature.takeRight(2).sum
      }, n)
    }
  }

  println(tribonacci(List(1, 1, 1), 10))
}
