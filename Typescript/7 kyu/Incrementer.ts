...
function incrementer(input_array: number[]) {
    var result:number[] = []    
    for (let index=0; index < input_array.length; index++)
    {let sum: number = input_array[index] + (index + 1)
        if (sum.toString().length > 1) result.push(Number(sum.toString().slice(-1)))
        else result.push(sum)
    }
    return result;
}

console.log(incrementer([1,5,9]));