...
def isPP(number):
    result = []
    for pair_one in range(2, number + 1):
        for pair_two in range(2, number + 1):
            if pair_one ** pair_two == number:
                result.append(pair_one)
                result.append(pair_two)
            break
    if len(result) == 0:
        return None
    else:
